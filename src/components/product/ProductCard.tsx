...
const ProductCard: React.FC<ProductCardProps> = ({ product, size = 'medium' }) => {
  ...
  const stockStatus = product.stockCount < 10 ? `Apenas ${product.stockCount} unidades restantes!` : null;
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden aspect-square bg-gray-50">
        ...
        {stockStatus && (
          <div className="absolute bottom-2 left-2 right-2 bg-brand-orange/90 text-white text-xs font-medium rounded-full px-3 py-1.5 text-center">
            {stockStatus}
          </div>
        )}
